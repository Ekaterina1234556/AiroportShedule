...
namespace AiroportShedule.Data
{
    public class AirportDbContext : IDisposable
    {
        ...
        private T ExecuteWithConnection<T>(Func<MySqlConnection, T> operation)
        {
            ...
            return ExecuteWithConnection(connection =>
            {
                ...
                if (reader2.Read())
                {
                    return new User
                    {
                        ...
                        Role = MapRoleIdToEnum(reader2.GetInt32("Role_ID"))
                    };
                }
        ...
        public bool AddCrewMember(string fullName, string role, int flightHours, int experience, string login, string password)
        {
            return ExecuteWithConnection(connection =>
            {
                var roleId = GetRoleIdByName(role, connection);
                var query = @"
        ...
        public bool AddHangar(string name, string type, string size)
        {
            return ExecuteWithConnection(connection =>
            {
                var typeId = GetHangarTypeIdByName(type, connection);
                var query = @"
        ...
        public bool DeleteHangar(int hangarId)
        {
            return ExecuteWithConnection(connection =>
            {
                ...
                {
                    checkCmd.Parameters.AddWithValue("@HangarID", hangarId);
                    var equipmentCount = Convert.ToInt32(checkCmd.ExecuteScalar());
        ...
        public bool DeleteRoute(int routeId)
        {
            return ExecuteWithConnection(connection =>
            {
                ...
                using (var checkCmd = new MySqlCommand(checkQuery, connection))
                {
                    ...
                    var scheduleCount = Convert.ToInt32(checkCmd.ExecuteScalar());
        ...
        public List<SupplyItem> GetSupplies()
        {
            ...
            using (var checkCmd = new MySqlCommand("SHOW TABLES LIKE 'supplies'", connection))
            {
                using var reader1 = checkCmd.ExecuteReader();
                if (!reader1.Read())
                {
                    reader1.Close();
                    CreateSuppliesTable(connection); 
                }
                else
                {
                    reader1.Close();
        ...
        private int GetHangarTypeIdByName(string typeName, MySqlConnection connection)
        {
        ...
        private int GetRoleIdByName(string roleName, MySqlConnection connection)
        {
        ...
        private void CreateSuppliesTable(MySqlConnection connection)
        {
            var createTable = @"
